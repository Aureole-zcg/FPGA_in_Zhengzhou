...
ram_dbl_16x2048	ram_dbl_16x2048_inst1 (
	.clock     ( clk       ),
	.data      ( q_ram_s   ),
	.rdaddress ( rdaddr    ),
	.rden      ( rden      ),
...
begin
    if (~rst_n)
    rden <= 1'b0;
    else if (cnt >= 13'd2049 && cnt <= 13'd4096)//2050~4096 rden拉高
        rden <= 1'b1;//读取
        else rden <= 1'b0;
end
...
begin
    if (~rst_n)
        ...
        begin
            if (cnt >= 13'd2049 && cnt <= 13'd3072)//2050~3073 rdaddr 1024~2047
                rdaddr <= cnt - 13'd1025;//1024~2047
            else if (cnt >= 13'd3073 && cnt <= 13'd4096)//3074~4097 rdaddr 0~1023
                rdaddr <= cnt - 13'd3073;//0~1023
                else rdaddr <= 11'd1023;  
        end
end
