...
parameter MS_MAX = 16'd3999;//秒和毫秒的计数峰值//localparam不能跨文件使用，为了仿真改为parameter
...
begin
    if (~rst_n)
    led_out<=1'b0;
    else if (clk_4s < clk_4ms>>2)//4s分成了999份，一份4ms,4ms分成了3999份，暗到亮的过程，分成1千份，所以s和ms的份数要对齐
